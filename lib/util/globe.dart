...
typedef void MapCreatedCallback(GlobeEarthController controller);
typedef void CameraPositionCallback(LatLon latLon, double zoom);

class GlobeEarth extends StatefulWidget {
  GlobeEarth({
    Key key,
    this.url,
    this.radius,
  ...
  @override
  _GlobeEarthState createState() => _GlobeEarthState();
}

class _GlobeEarthState extends State<GlobeEarth> with TickerProviderStateMixin {
  GlobeEarthController _controller;
  double width;
  double height;
  double zoom;
  double _lastZoom;
  ...
  final double _radius = 512 / (2 * math.pi);
  ...
  final double tileWidth = 512;
  final double tileHeight = 512;
  final int minZoom = 2;
  ...
  void initState() {
    ...
    _controller = GlobeEarthController(this);
...
class GlobeEarthController {
  GlobeEarthController(this._state);

  final _GlobeEarthState _state;
